...
if(isset($_POST['submit'])){
   ...
   $update_profile = $conn->prepare("UPDATE `vrijwilligers` SET name = ?, email = ? WHERE id = ?");
   $update_profile->execute([$name, $email, $vrijwilliger_id]);
   ...
   if($old_pass == $empty_pass){
      ...
      if($new_pass != $empty_pass){
         $update_admin_pass = $conn->prepare("UPDATE `vrijwilligers` SET password = ? WHERE id = ?");
         $update_admin_pass->execute([$cpass, $vrijwilliger_id]);
